...
export function GalleryImageUploader({ galleryId }: Props) {
  ...
  const onDrop = useCallback(async (files: File[]) => {
    ...
    await Promise.all(
      files.map(async (file) => {
        ...
        await fetch("/api/admin/galleries/upload", { method: "POST", body: formData });
        completed++;
  ...
  return (
    <div>
      <h2 className="text-sm font-medium text-[#1A1A18] mb-3">Upload Images</h2>
      <div
        {...getRootProps()}

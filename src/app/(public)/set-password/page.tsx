...
function SetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!oobCode) {
      setErrorMsg("Invalid or missing invite link. Please request a new one.");
      return;
    }
    ...
    try {
      ...
      if (code === "auth/expired-action-code") {
        setErrorMsg("This invite link has expired. Please contact your photographer for a new one.");
  ...
  if (status === "success") {
    return (
      <div className="text-center">
        <h2 className="font-serif text-2xl mb-4">Password set!</h2>
        <p className="text-[#6B6860] text-sm leading-relaxed mb-8">
          Your password has been created. You can now log in to view your gallery.
        </p>
        <Link
          href="/login"
          className="inline-block py-4 px-10 bg-[#1A1A18] text-white text-xs tracking-widest uppercase hover:bg-[#C4A882] transition-colors"
        >
          Go to login
        </Link>
      </div>
  ...
  return (
    <>
      <h2 className="font-serif text-2xl mb-2">Welcome! Set your password</h2>
